# server.py
import socket
import sys
import json
import base64
import hashlib
import random
import csv
import os
import time
from Crypto.Random import get_random_bytes
from Crypto.Util.number import getRandomNBitInteger
from Crypto.Cipher import AES
from Crypto.Util.Padding import pad, unpad

# function to perform modular exponentiation efficiently
def modexp(base, exponent, modulus):
    # initialize result to 1 (identity of multiplication)
    result = 1
    # reduce base modulo modulus initially
    base = base % modulus
    # loop while exponent is greater than zero
    while exponent > 0:
        # if the current least significant bit of exponent is 1, multiply result by base modulo modulus
        if exponent & 1:
            result = (result * base) % modulus
        # right shift exponent by 1 bit (divide by 2)
        exponent >>= 1
        # square the base modulo modulus for next iteration
        base = (base * base) % modulus
    # return the final computed result (base^exponent mod modulus)
    return result

# helper function to convert integer to bytes (big endian)
def to_bytes(x):
    # calculate minimum number of bytes needed and convert
    return x.to_bytes((x.bit_length() + 7) // 8, 'big')

# rsa keys and diffie-hellman parameters hardcoded for server (bob) and client (alice)

# bob's rsa modulus (n), public exponent (e), and private exponent (d)
n_bob = 
e_bob = 65537
d_bob = int(os.environ["DEMO_RSA_PRIVATE_EXPONENT"])
n_alice = 
e_alice = 65537

# diffie-hellman prime modulus p (2048 bit prime from RFC3526) hardcoded as integer
p = int("""

""".replace("\n",""), 16)

# generator g for diffie-hellman group
g = 2

# server's ip id
ID_B = '192.168.1.14'
# ip and port to listen on
LISTEN_IP = '0.0.0.0'
LISTEN_PORT = 5000

# create a socket object for server
sock = socket.socket()
# bind socket to listen ip and port
sock.bind((LISTEN_IP, LISTEN_PORT))
# start listening for incoming connections (max 1)
sock.listen(1)
print("[server] listening on port", LISTEN_PORT)

# accept a new client connection
conn, addr = sock.accept()
print(f"[server] connection from {addr}")

# leaderboard and history files for game statistics
leaderboard_file = "leaderboard.csv"
history_file = "history.txt"
# leaderboard dictionary to hold players' scores and attempts
leaderboard = {}

# if leaderboard file exists, load it into memory
if os.path.exists(leaderboard_file):
    with open(leaderboard_file, "r") as f:
        for row in csv.reader(f):
            # each row should have name, attempts, score
            if len(row) == 3:
                name, attempts, score = row
                # store in leaderboard dict with int conversions
                leaderboard[name] = {"attempts": int(attempts), "score": int(score)}

# if history file does not exist, create an empty one
if not os.path.exists(history_file):
    open(history_file, 'w').close()

# helper function to append a new game result to history file
def append_to_profile(name, attempts, duration):
    with open(history_file, "a") as f:
        f.write(f"{name} | attempts: {attempts} | time: {duration:.2f}s\n")

# helper function to read the entire history file
def read_profile():
    with open(history_file, "r") as f:
        return f.read()

# helper function to remove a player from the leaderboard and update the file
def remove_leaderboard_entry(player):
    if player in leaderboard:
        del leaderboard[player]
        with open(leaderboard_file, "w", newline='') as f:
            # sort leaderboard by attempts asc, score desc
            sorted_board = sorted(leaderboard.items(), key=lambda x: (x[1]["attempts"], -x[1]["score"]))
            # write updated leaderboard back to file
            csv.writer(f).writerows([[name, data["attempts"], data["score"]] for name, data in sorted_board])
        return True
    return False

# perform the secure handshake protocol for mutual authentication and key exchange
def perform_handshake(conn, ID_B):
    # receive initial client handshake message (json)
    data1 = conn.recv(8192)
    init = json.loads(data1.decode())

    # extract client's diffie-hellman public value A (g^a mod p)
    A  = int(init['A'])
    # extract client's challenge RA (random bytes 32 bytes)
    RA = base64.b64decode(init['RA'])
    # extract client's id string
    ID_A = init['ID']
    # extract client's session iv (initialization vector for aes)
    session_iv = base64.b64decode(init['IV'])
    print(f"[server] received session iv from client: {session_iv.hex()}")

    # generate server's private diffie-hellman exponent b (2048 bit secure random int)
    b = getRandomNBitInteger(2048)
    print(f"[server] exponent b selected: {b}")  # print chosen exponent b for logging/debugging

    # compute server's diffie-hellman public value B = g^b mod p
    B = modexp(g, b, p)
    # generate server's challenge RB (random 32 bytes)
    RB = get_random_bytes(32)

    # compute hash H over concatenated data including A, B, RA, RB, IDs and session iv
    concat = to_bytes(A) + to_bytes(B) + RA + RB + ID_A.encode() + ID_B.encode() + session_iv
    H_bytes = hashlib.sha256(concat).digest()
    # convert hash bytes to integer for signing
    H_int = int.from_bytes(H_bytes, 'big')
    # sign hash with server's private key d_bob (modular exponentiation)
    Sig_B = modexp(H_int, d_bob, n_bob)

    # create response dictionary with B, RB, ID_B, H and signature
    SB = {
        'B': str(B),
        'RB': base64.b64encode(RB).decode(),
        'ID': ID_B,
        'H': H_bytes.hex(),
        'Sig': str(Sig_B)
    }
    # send server response to client (json encoded)
    conn.send(json.dumps(SB).encode())

    # receive client's signature on hash H (json)
    data2 = conn.recv(8192)
    sa = json.loads(data2.decode())
    Sig_A = int(sa['Sig'])

    # verify client's signature on H using alice's public key
    if modexp(Sig_A, e_alice, n_alice) != H_int:
        # if verification fails, print message and send notification to client before closing
        print("[server] alice authentication failed. terminating game round.")
        conn.sendall(b"alice authentication failed. terminating game round.")  # notify client
        conn.close()
        sys.exit(1)  # terminate server process due to failed authentication

    # if verification succeeds, print success message
    print("[server] alice authentication succeeded.")

    # compute shared secret g^(ab) mod p for dh key exchange
    shared = modexp(A, b, p)
    # derive session key k by hashing shared secret with sha256
    K = hashlib.sha256(to_bytes(shared)).digest()

    # clear sensitive variables
    b = None
    shared = None

    # return the session key, iv, and client's id
    return K, session_iv, ID_A

# encrypt a plaintext message using aes-256-cbc with key and iv
def encrypt_message(message, key, iv):
    cipher = AES.new(key, AES.MODE_CBC, iv)
    # pad message to block size and encrypt
    return cipher.encrypt(pad(message.encode(), AES.block_size))

# decrypt a ciphertext message using aes-256-cbc with key and iv
def decrypt_message(ciphertext, key, iv):
    cipher = AES.new(key, AES.MODE_CBC, iv)
    # decrypt and unpad to recover plaintext string
    return unpad(cipher.decrypt(ciphertext), AES.block_size).decode()

# send an encrypted message to the client connection and print debug info
def send_encrypted(conn, message, key, iv, label="server"):
    ciphertext = encrypt_message(message, key, iv)
    print(f"[{label}] ciphertext: {ciphertext.hex()}")  # print encrypted data hex for debugging
    conn.sendall(ciphertext)  # send encrypted bytes over socket
    print(f"[{label}] sending plaintext: {message}")  # print plaintext for debugging

# receive an encrypted message from client connection and decrypt it
def receive_encrypted(conn, key, iv):
    data = conn.recv(8192)
    decrypted = decrypt_message(data, key, iv)
    print(f"[server] decrypted plaintext: {decrypted}")  # print decrypted message
    return decrypted

print("[server] starting session")

try:
    # main server loop waiting for client connections and interactions
    while True:
        print("[server] waiting for handshake...")
        # perform dh handshake, mutual authentication, get session key and iv
        K, session_iv, ID_A = perform_handshake(conn, ID_B)

        while True:
            print("[server] waiting for player name...")
            # receive encrypted player name from client
            player_name = receive_encrypted(conn, K, session_iv).strip()
            print(f"[server] player name received: {player_name}")

            # send welcome message encrypted to client
            send_encrypted(conn, f"welcome to the guessing game, {player_name}!", K, session_iv)
            # get player's current total score or 0 if new
            total_score = leaderboard.get(player_name, {}).get("score", 0)

            while True:
                # send menu options encrypted to client
                send_encrypted(conn, "\n--- menu ---\n1. start new game\n2. view leaderboard\n3. view history\n4. exit\nenter your choice:", K, session_iv)
                # receive menu choice from client
                choice = receive_encrypted(conn, K, session_iv).strip()

                if choice == '4':
                    # client chose exit, send goodbye message encrypted
                    send_encrypted(conn, f"goodbye {player_name}!", K, session_iv)
                    break

                elif choice == '2':
                    # client wants to view leaderboard, prepare sorted leaderboard string
                    board = sorted(leaderboard.items(), key=lambda x: (x[1]['attempts'], -x[1]['score']))
                    msg = "\nleaderboard:\n" + "\n".join([f"{i+1}. {n} - attempts: {d['attempts']}, score: {d['score']}" for i, (n, d) in enumerate(board)])
                    msg += "\noptions:\nd - delete an entry\nb - back to main menu"
                    send_encrypted(conn, msg, K, session_iv)
                    # receive client action for leaderboard
                    action = receive_encrypted(conn, K, session_iv).strip().upper()
                    if action == 'D':
                        # client wants to delete an entry, prompt for name
                        send_encrypted(conn, "enter the name to delete:", K, session_iv)
                        del_name = receive_encrypted(conn, K, session_iv).strip()
                        if remove_leaderboard_entry(del_name):
                            send_encrypted(conn, f"{del_name} removed.", K, session_iv)
                        else:
                            send_encrypted(conn, f"{del_name} not found.", K, session_iv)

                elif choice == '3':
                    # client wants to view game history
                    history = read_profile()
                    send_encrypted(conn, "\ngame history:\n" + (history if history else "no history yet."), K, session_iv)
                    send_encrypted(conn, "press enter to return...", K, session_iv)
                    receive_encrypted(conn, K, session_iv)  # wait for enter

                elif choice == '1':
                    # client wants to start a new game round, perform new handshake for fresh keys
                    print("[server] starting new game round handshake for fresh keys...")
                    K, session_iv, _ = perform_handshake(conn, ID_B)

                    # ask client if timed mode enabled
                    send_encrypted(conn, "enable timed mode? (y/n):", K, session_iv)
                    timed = receive_encrypted(conn, K, session_iv).lower() == 'y'

                    # ask client for difficulty level
                    send_encrypted(conn, "choose difficulty: 1. easy (1-50), 2. medium (1-100), 3. hard (1-200):", K, session_iv)
                    level = receive_encrypted(conn, K, session_iv)
                    # map difficulty to max number range
                    max_range = {'1': 50, '2': 100, '3': 200}.get(level, 100)
                    # pick a random secret number in range for guessing game
                    number = random.randint(1, max_range)
                    send_encrypted(conn, f"i picked a number between 1 and {max_range}. start guessing!", K, session_iv)

                    # initialize attempts and invalid guess counters and start time
                    attempts, invalid = 0, 0
                    start = time.time()

                    while True:
                        # prompt client for a guess
                        send_encrypted(conn, "enter your guess:", K, session_iv)
                        guess = receive_encrypted(conn, K, session_iv)

                        # check if timed mode expired
                        if timed and time.time() - start > 30:
                            send_encrypted(conn, "time's up!", K, session_iv)
                            break

                        # validate guess input format and range
                        if not guess.isdigit() or not (1 <= int(guess) <= max_range):
                            send_encrypted(conn, "invalid input.", K, session_iv)
                            invalid += 1
                            if invalid >= 10:
                                send_encrypted(conn, "too many invalid inputs.", K, session_iv)
                                break
                            continue

                        # convert guess to int and increment attempts
                        guess = int(guess)
                        attempts += 1

                        # provide hints and feedback based on guess relative to secret number
                        if guess < number:
                            hint = "you're burning hot!" if number - guess <= 5 else "you're warm." if number - guess <= 10 else "you're cold."
                            send_encrypted(conn, f"higher. {hint}", K, session_iv)
                        elif guess > number:
                            hint = "you're burning hot!" if guess - number <= 5 else "you're warm." if guess - number <= 10 else "you're cold."
                            send_encrypted(conn, f"lower. {hint}", K, session_iv)
                        else:
                            # guess is correct, compute duration and update stats
                            duration = time.time() - start
                            msg = f"correct! attempts: {attempts}, time: {duration:.2f}s"
                            append_to_profile(player_name, attempts, duration)
                            # calculate score based on attempts (max 10 points minus attempts)
                            score = max(0, 10 - attempts)
                            total_score += score
                            # update leaderboard with best attempts and total score
                            leaderboard[player_name] = {
                                "attempts": min(attempts, leaderboard.get(player_name, {}).get("attempts", attempts)),
                                "score": total_score
                            }
                            # save leaderboard to file sorted by attempts ascending and score descending
                            with open(leaderboard_file, "w", newline='') as f:
                                writer = csv.writer(f)
                                for name, data in sorted(leaderboard.items(), key=lambda x: (x[1]["attempts"], -x[1]["score"])):
                                    writer.writerow([name, data["attempts"], data["score"]])
                            msg += f"\nscore this round: {score}, total: {total_score}"
                            send_encrypted(conn, msg, K, session_iv)
                            break

                else:
                    # invalid menu option, send error message
                    send_encrypted(conn, "invalid option.", K, session_iv)

except Exception as e:
    # handle exceptions silently (could be improved for debugging)
    print(f"")

finally:
    # clean up connections and sockets on exit
    conn.close()
    sock.close()
    print("[server] connection closed.")
